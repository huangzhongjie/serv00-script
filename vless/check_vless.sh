...
save_config() {
    local port=$1
    if [[ ! -f ~/domains/serv00-script/vless/config.json ]]; then
        uuid=$(generate_uuid)
        cat <<EOL > ~/domains/serv00-script/vless/config.json
{
    "uuid": "$uuid",
    "port": $port
}
EOL
        ...
        jq --arg port "$port" '.port = ($port | tonumber)' ~/domains/serv00-script/vless/config.json > ~/domains/serv00-script/vless/config_tmp.json && mv ~/domains/serv00-script/vless/config_tmp.json ~/domains/serv00-script/vless/config.json
...
deploy_vless() {
    ...
    npm install
    # 启动vless项目
    ~/.npm-global/bin/pm2 start ~/domains/serv00-script/vless/app.js --name vless
    # 保存pm2进程状态
    ...
    echo -e "域名: ${GREEN}aniange5.cloudns.ch${NC}"
    echo -e "vless进程维护定时任务脚本: ${GREEN}cd ~/domains/serv00-script/vless && ./check_vless.sh${NC}"
    echo -e "VLESS节点信息: ${GREEN}vless://${uuid}@aniange5.cloudns.ch:${port}?flow=&security=none&encryption=none&type=ws&host=aniange5.cloudns.ch&path=/&sni=&fp=&pbk=&sid=#aniange5.cloudns.ch${NC}"
}
...
main() {
    ...
    if [ "$port_provided" = true ]; then
        ...
        if [[ -f config.json ]]; then
            ...
            echo -e "UUID: ${uuid}"
            echo -e "Port: ${port}"
            echo -e "域名: aniange5.cloudns.ch"
            echo -e "VLESS节点信息: vless://${uuid}@aniange5.cloudns.ch:${port}?flow=&security=none&encryption=none&type=ws&host=aniange5.cloudns.ch&path=/&sni=&fp=&pbk=&sid=#aniange5.cloudns.ch"
